...
lambda0 = 0.01*np.ones(nrows[0]**2)

out_dir = 'patch_increments_cameraman_nh'
...
with open(summary_table_dir, 'w+') as f:
    ...
    for r in nrows:
        ...
        cmd = f'python ../../regularization_learning.py ../../datasets/cameraman_128_5_nh/filelist.txt -t patch -prows {str(r)} -pdata {str(lambda0.tolist()).strip("[]").replace(",","")} -o {os.path.join(out_dir,str(r))} -v'
        print(cmd[:150])
        os.system(cmd)
