...
def find_optimal_data_patch(dsfile,initial_data_parameter:Patch,show=False):
    ...
    optimal = nsdogbox(fun=lambda x: data_cost_fn_patch(dsfile, x), grad=lambda x: data_gradient_fn_patch(dsfile, x), reg_grad=lambda x: smooth_data_gradient_fn_patch(
        dsfile, x), x0=initial_data_parameter, verbose=iprint, initial_radius=np.min(initial_data_parameter.data), max_radius=1000, lb=1.0)
    # np.linalg.norm(initial_data_parameter.data,np.inf)

    if show == True:
        print(optimal)
    x = optimal.x
    optimal_ds = patch_denoise_ds(dsfile, data_parameter=x, reg_parameter=OnesPatch(x.px, x.py),niter=2000, show=False)
    return optimal,optimal_ds
...
def reg_cost_fn_patch(dsfile,reg_parameter):
    den_ds = patch_denoise_ds(dsfile,data_parameter=OnesPatch(reg_parameter.px,reg_parameter.py),reg_parameter=reg_parameter,niter=2000)
    return l2_cost_ds(den_ds)
...
def smooth_reg_gradient_fn_patch(dsfile,reg_parameter,gamma=100000):
    den_ds = patch_denoise_ds(dsfile,data_parameter=OnesPatch(reg_parameter.px,reg_parameter.py),reg_parameter=reg_parameter,niter=2000)
    return smooth_patch_reg_gradient_ds(den_ds,reg_parameter,gamma=gamma)


def find_optimal_reg_patch(dsfile, initial_reg_parameter, show=False, gamma=100000, threshold_radius=1e-4):
    iprint = 0
    if show == True:
        iprint = 2
    optimal = nsdogbox(fun=lambda x: reg_cost_fn_patch(dsfile,x),grad=lambda x:reg_gradient_fn_patch(dsfile,x),reg_grad=lambda x:smooth_reg_gradient_fn_patch(dsfile,x,gamma=gamma),x0=initial_reg_parameter,verbose=iprint,initial_radius=0.01,threshold_radius=threshold_radius)
    if show == True:
        print(optimal)
    x = optimal.x
    optimal_ds = patch_denoise_ds(dsfile,data_parameter=OnesPatch(x.px,x.py),reg_parameter=x,niter=2000)
    return optimal,optimal_ds