...
def find_optimal_reg_scalar(dsfile,initial_reg_parameter,show=False):
    ...
    optimal = scipy.optimize.minimize(fun=lambda x: reg_cost_fn_scalar(dsfile,x),jac=lambda x:reg_gradient_fn_scalar(dsfile,x),x0=initial_reg_parameter,method='trust-constr',bounds=bnds,options={'verbose':iprint})
    if show == True:
        print(optimal)
